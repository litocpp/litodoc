...
auto load_book_dataset(ref<rstd::path::Path> root)
  ...
  for (const auto &descriptor : *descriptors) {
    ...
    if (rstd::crypto::sha256_hex(page_text.as_str()).as_str() !=
        digest.as_str())
      return Err(
...
auto publish_book_dataset(ref<rstd::path::Path> root,
  ...
  for (const auto &page : dataset.pages) {
    ...
    entry.insert(
        ...
        book_json_string(rstd::crypto::sha256_hex(text.as_str()).as_str()));
  ...
  summary.digest = rstd::crypto::sha256_hex(text.as_str());
  summary.manifest =
