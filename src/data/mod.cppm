...
auto data_path(ref<rstd::path::Path> root, ref<str> relative)
    ...
    return Ok(rstd::path::PathBuf::from(root).join(rstd::path::PathBuf::from(relative).as_path()));
}

auto write_data_file(ref<rstd::path::Path> root, ref<str> relative, ref<str> contents)
    -> Result<empty, String> {
    auto path = data_path(root, relative);
    if (path.is_err()) return Err(rstd::move(path).unwrap_err());
    auto parent = path->as_path().parent();
    if (parent.is_none()) return Err(rstd::format("doc data '{}' has no parent", path->as_path()));
    ...
    if (written.is_err())
        return Err(rstd::format(
            "cannot write doc data '{}': {}", path->as_path(), rstd::move(written).unwrap_err()));
    return Ok(empty {});
}
...
auto encode_source_fragment(ref<str> package, const Source& source) -> Json {
    ...
    root.insert(String::make("contents"_str), json_string(source.contents.as_str()));
...
auto encode_package(const Package& package) -> Json {
    ...
    for (const auto& symbol : package.symbols) {
        ...
        object.insert(String::make("module-page"_str), json_string(symbol.module_page.as_str()));
    ...
    for (const auto& source : package.sources) {
        auto fragment      = json_text(encode_source_fragment(package.name.as_str(), source));
        auto fragment_path = source_data_path(package.name.as_str(), source.path.as_str());
        auto object        = JsonMap::make();
        ...
        object.insert(String::make("data"_str), json_string(fragment_path.as_str()));
...
auto required_string(const Json& value, ref<str> name, ref<str> context) -> Result<String, String> {
...
auto optional_string(const Json& value, ref<str> name, ref<str> context)
    ...
    auto member = required_member(value, name, context);
    if (member.is_err()) return Err(rstd::move(member).unwrap_err());
    if ((**member).is_null()) return Ok(Option<String> {});
    auto text = (**member).as_str();
...
auto required_bool(const Json& value, ref<str> name, ref<str> context) -> Result<bool, String> {
    auto member = required_member(value, name, context);
    if (member.is_err()) return Err(rstd::move(member).unwrap_err());
    auto boolean = (**member).as_bool();
    if (boolean.is_none()) return Err(rstd::format("{}.{} must be a boolean", context, name));
    return Ok(*boolean);
}
...
auto expect_header(const Json& value, ref<str> format, usize version, ref<str> context)
    -> Result<empty, String> {
    ...
    if (actual_format.is_err()) return Err(rstd::move(actual_format).unwrap_err());
    if (actual_version.is_err()) return Err(rstd::move(actual_version).unwrap_err());
    if (actual_format->as_str() != format)
        return Err(rstd::format(
            "{} has format '{}', expected '{}'", context, actual_format->as_str(), format));
    if (*actual_version != version)
        return Err(rstd::format("{} has unsupported version {}", context, *actual_version));
    return Ok(empty {});
}
...
auto parse_kind(ref<str> value) -> Option<frontend::DeclarationKind> {
    ...
    if (value == "enum"_str) return Some(DeclarationKind::Enum);
    if (value == "concept"_str) return Some(DeclarationKind::Concept);
    if (value == "alias"_str) return Some(DeclarationKind::Alias);
    if (value == "function"_str) return Some(DeclarationKind::Function);
    if (value == "variable"_str) return Some(DeclarationKind::Variable);
    if (value == "field"_str) return Some(DeclarationKind::Field);
    return None();
}

auto parse_json_text(ref<str> contents, ref<str> context) -> Result<Json, String> {
    auto parsed = rstd::json::from_str(contents);
    if (parsed.is_err())
        return Err(rstd::format("invalid {} JSON: {}", context, parsed.unwrap_err()));
    if (! parsed->is_object()) return Err(rstd::format("{} must be a JSON object", context));
    return Ok(rstd::move(parsed).unwrap());
}

auto read_data_text(ref<rstd::path::Path> root, ref<str> relative) -> Result<String, String> {
    auto path = data_path(root, relative);
    if (path.is_err()) return Err(rstd::move(path).unwrap_err());
    auto contents = rstd::fs::read_to_string(path->as_path());
    if (contents.is_err())
        return Err(rstd::format(
            "cannot read doc data '{}': {}", path->as_path(), rstd::move(contents).unwrap_err()));
    return Ok(rstd::move(contents).unwrap());
}

auto decode_source(ref<rstd::path::Path> root,
                   ref<str>              package_name,
                   const Json&           descriptor,
                   const Json*           manifest_descriptor) -> Result<Source, String> {
    auto path   = required_string(descriptor, "path"_str, "doc source descriptor"_str);
    auto page   = required_string(descriptor, "page"_str, "doc source descriptor"_str);
    auto data   = required_string(descriptor, "data"_str, "doc source descriptor"_str);
    auto digest = required_string(descriptor, "digest"_str, "doc source descriptor"_str);
    if (path.is_err()) return Err(rstd::move(path).unwrap_err());
    if (page.is_err()) return Err(rstd::move(page).unwrap_err());
    ...
    if (manifest_descriptor != nullptr) {
        auto manifest_data =
            required_string(*manifest_descriptor, "path"_str, "doc manifest source"_str);
        auto manifest_digest =
            required_string(*manifest_descriptor, "digest"_str, "doc manifest source"_str);
        if (manifest_data.is_err()) return Err(rstd::move(manifest_data).unwrap_err());
        if (manifest_digest.is_err()) return Err(rstd::move(manifest_digest).unwrap_err());
        if (manifest_data->as_str() != data->as_str() ||
            manifest_digest->as_str() != digest->as_str())
            return Err(rstd::format("doc source manifest mismatch for '{}'", path->as_str()));
    }
    ...
    auto fragment_text    = required_string(*parsed, "contents"_str, "doc source"_str);
    if (fragment_package.is_err()) return Err(rstd::move(fragment_package).unwrap_err());
    if (fragment_path.is_err()) return Err(rstd::move(fragment_path).unwrap_err());
    if (fragment_page.is_err()) return Err(rstd::move(fragment_page).unwrap_err());
    if (fragment_text.is_err()) return Err(rstd::move(fragment_text).unwrap_err());
    if (fragment_package->as_str() != package_name || fragment_path->as_str() != path->as_str() ||
        fragment_page->as_str() != page->as_str())
        return Err(rstd::format("doc source '{}' identity mismatch", data->as_str()));
    return Ok(Source {
...
auto decode_package(const Json&                   document,
    ...
    auto name        = required_string(**metadata, "name"_str, "doc package metadata"_str);
    auto version     = required_string(**metadata, "version"_str, "doc package metadata"_str);
    auto root_module = required_string(**metadata, "root-module"_str, "doc package metadata"_str);
    auto profile     = required_string(**metadata, "profile"_str, "doc package metadata"_str);
    auto toolchain_version =
        required_string(**generator, "toolchain-version"_str, "doc generator"_str);
    auto toolchain_target =
        required_string(**generator, "toolchain-target"_str, "doc generator"_str);
    auto language_standard =
        required_string(**generator, "language-standard"_str, "doc generator"_str);
    ...
    if (toolchain_version.is_err()) return Err(rstd::move(toolchain_version).unwrap_err());
    if (toolchain_target.is_err()) return Err(rstd::move(toolchain_target).unwrap_err());
    if (language_standard.is_err()) return Err(rstd::move(language_standard).unwrap_err());
    ...
    if (modules.is_err()) return Err(rstd::move(modules).unwrap_err());
    for (const auto& value : **modules) {
        ...
        if (module_name.is_err()) return Err(rstd::move(module_name).unwrap_err());
        if (module_page.is_err()) return Err(rstd::move(module_page).unwrap_err());
        if (comment.is_err()) return Err(rstd::move(comment).unwrap_err());
        if (reexports.is_err()) return Err(rstd::move(reexports).unwrap_err());
        auto module = Module {
        ...
        for (const auto& reexport : **reexports) {
            ...
            if (text.is_none()) return Err(String::make("doc reexport must be a string"_str));
    ...
    if (symbols.is_err()) return Err(rstd::move(symbols).unwrap_err());
    for (const auto& value : **symbols) {
        ...
        auto module_page    = required_string(value, "module-page"_str, "doc symbol"_str);
        ...
        if (kind.is_none())
            return Err(rstd::format("unknown doc declaration kind '{}'", kind_text->as_str()));
        auto source_path     = required_string(**source, "path"_str, "doc symbol source"_str);
        auto source_page     = required_string(**source, "page"_str, "doc symbol source"_str);
        auto source_line     = required_usize(**source, "line"_str, "doc symbol source"_str);
        auto source_column   = required_usize(**source, "column"_str, "doc symbol source"_str);
        auto source_end_line = required_usize(**source, "end-line"_str, "doc symbol source"_str);
        ...
        if (source_page.is_err()) return Err(rstd::move(source_page).unwrap_err());
        if (source_line.is_err()) return Err(rstd::move(source_line).unwrap_err());
        if (source_column.is_err()) return Err(rstd::move(source_column).unwrap_err());
        if (source_end_line.is_err()) return Err(rstd::move(source_end_line).unwrap_err());
        if (source_end_column.is_err()) return Err(rstd::move(source_end_column).unwrap_err());
    ...
    if (manifest_sources.is_some() && (**manifest_sources).len() != (**sources).len())
        return Err(rstd::format("doc package '{}' source manifest length mismatch",
                                package.name.as_str()));
    if (root.is_some()) {
        for (auto index = usize {}; index < (**sources).len(); ++index) {
            const auto* expected =
                manifest_sources.is_some() ? rstd::addressof((**manifest_sources)[index]) : nullptr;
            auto source = decode_source(*root, package.name.as_str(), (**sources)[index], expected);
            if (source.is_err()) return Err(rstd::move(source).unwrap_err());
            package.sources.push(rstd::move(source).unwrap());
        }
    } else {
        for (const auto& value : **sources) {
            auto path   = required_string(value, "path"_str, "doc source descriptor"_str);
            auto page   = required_string(value, "page"_str, "doc source descriptor"_str);
            auto data   = required_string(value, "data"_str, "doc source descriptor"_str);
            auto digest = required_string(value, "digest"_str, "doc source descriptor"_str);
            if (path.is_err()) return Err(rstd::move(path).unwrap_err());
            if (page.is_err()) return Err(rstd::move(page).unwrap_err());
            if (data.is_err()) return Err(rstd::move(data).unwrap_err());
            if (digest.is_err()) return Err(rstd::move(digest).unwrap_err());
            if (! safe_data_path(data->as_str()))
                return Err(rstd::format("invalid doc source data path '{}'", data->as_str()));
    ...
    auto diagnostics = required_array(document, "diagnostics"_str, "doc package"_str);
    if (diagnostics.is_err()) return Err(rstd::move(diagnostics).unwrap_err());
    for (const auto& value : **diagnostics) {
        auto severity = required_string(value, "severity"_str, "doc diagnostic"_str);
        ...
        if (severity.is_err()) return Err(rstd::move(severity).unwrap_err());
        if (code.is_err()) return Err(rstd::move(code).unwrap_err());
        if (message.is_err()) return Err(rstd::move(message).unwrap_err());
        if (path.is_err()) return Err(rstd::move(path).unwrap_err());
    ...
    if (coverage.is_err()) return Err(rstd::move(coverage).unwrap_err());
    auto documented   = required_usize(**coverage, "documented"_str, "doc coverage"_str);
    auto undocumented = required_usize(**coverage, "undocumented"_str, "doc coverage"_str);
    auto unsupported  = required_usize(**coverage, "unsupported"_str, "doc coverage"_str);
    if (documented.is_err()) return Err(rstd::move(documented).unwrap_err());
    if (undocumented.is_err()) return Err(rstd::move(undocumented).unwrap_err());
    if (unsupported.is_err()) return Err(rstd::move(unsupported).unwrap_err());
...
auto dataset_manifest_json(const Dataset& dataset) -> String {
    ...
    for (const auto& package : dataset.packages) {
        ...
        for (const auto& source : package.sources) {
            auto text  = json_text(encode_source_fragment(package.name.as_str(), source));
            auto entry = JsonMap::make();
            entry.insert(
                String::make("path"_str),
                json_string(
                    source_data_path(package.name.as_str(), source.path.as_str()).as_str()));
        ...
        entry.insert(String::make("digest"_str), json_string(data_digest(text.as_str()).as_str()));
        entry.insert(String::make("sources"_str), Json::Array(rstd::move(source_entries)));
...
auto summarize_dataset(ref<rstd::path::Path> root, const Dataset& dataset) -> DataSummary {
...
auto load_dataset(ref<rstd::path::Path> root) -> Result<Dataset, String> {
    ...
    if (manifest_text.is_err()) return Err(rstd::move(manifest_text).unwrap_err());
    ...
    if (title.is_err()) return Err(rstd::move(title).unwrap_err());
    if (default_package.is_err()) return Err(rstd::move(default_package).unwrap_err());
    auto packages = required_array(*manifest, "packages"_str, "doc dataset manifest"_str);
    if (packages.is_err()) return Err(rstd::move(packages).unwrap_err());
    ...
    for (const auto& entry : **packages) {
        ...
        auto digest  = required_string(entry, "digest"_str, "doc manifest package"_str);
        auto sources = required_array(entry, "sources"_str, "doc manifest package"_str);
        if (name.is_err()) return Err(rstd::move(name).unwrap_err());
        if (path.is_err()) return Err(rstd::move(path).unwrap_err());
        if (digest.is_err()) return Err(rstd::move(digest).unwrap_err());
        if (sources.is_err()) return Err(rstd::move(sources).unwrap_err());
        ...
        if (! safe_data_path(path->as_str()))
            return Err(rstd::format("invalid doc package data path '{}'", path->as_str()));
        if (! paths.insert(path->clone()))
            return Err(rstd::format("duplicate doc data path '{}'", path->as_str()));
        for (const auto& source : **sources) {
            auto source_path = required_string(source, "path"_str, "doc manifest source"_str);
            if (source_path.is_err()) return Err(rstd::move(source_path).unwrap_err());
            if (! safe_data_path(source_path->as_str()))
                return Err(
                    rstd::format("invalid doc source data path '{}'", source_path->as_str()));
            if (! paths.insert(source_path->clone()))
                return Err(rstd::format("duplicate doc data path '{}'", source_path->as_str()));
        }
        auto package_text = read_data_text(root, path->as_str());
        if (package_text.is_err()) return Err(rstd::move(package_text).unwrap_err());
        auto actual_digest = data_digest(package_text->as_str());
        if (actual_digest.as_str() != digest->as_str())
            return Err(rstd::format("doc package '{}' digest mismatch", name->as_str()));
        ...
        if (package->name.as_str() != name->as_str())
            return Err(rstd::format("doc package '{}' identity mismatch", name->as_str()));
        dataset.packages.push(rstd::move(package).unwrap());
    }
    if (default_package->is_some() && ! names.contains((**default_package).as_str()))
...
auto publish_dataset(ref<rstd::path::Path> output, const Dataset& dataset)
    ...
    for (const auto& package : dataset.packages) {
        for (const auto& source : package.sources) {
            auto relative = source_data_path(package.name.as_str(), source.path.as_str());
            auto text     = json_text(encode_source_fragment(package.name.as_str(), source));
            auto written =
                write_data_file(publication->staging.as_path(), relative.as_str(), text.as_str());
            if (written.is_err()) {
    ...
    auto manifest_text = dataset_manifest_json(dataset);
    auto written       = write_data_file(
        publication->staging.as_path(), "manifest.json"_str, manifest_text.as_str());
    if (written.is_err()) {
        (void)abort_publication(*publication);
        return Err(rstd::move(written).unwrap_err());
    }
