...
auto remove_publication_directory(ref<rstd::path::Path> path) -> Result<empty, String> {
...
auto abort_publication(const Publication& publication) -> Result<empty, String> {
    return remove_publication_directory(publication.staging.as_path());
}

auto commit_publication(const Publication& publication) -> Result<empty, String> {
    ...
    if (*output_exists) {
        auto moved = rstd::fs::rename(publication.output.as_path(), publication.backup.as_path());
        if (moved.is_err())
            return Err(rstd::format("cannot preserve previous doc output '{}': {}",
                                    publication.output.as_path(),
                                    rstd::move(moved).unwrap_err()));
    }
    auto published = rstd::fs::rename(publication.staging.as_path(), publication.output.as_path());
    if (published.is_err()) {
        if (*output_exists)
            (void)rstd::fs::rename(publication.backup.as_path(), publication.output.as_path());
    ...
    if (*output_exists) {
        ...
        if (cleaned.is_err()) return cleaned;
    }
    return Ok(empty {});
}
