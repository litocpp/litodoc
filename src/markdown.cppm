...
auto find_byte(ref<str> value, usize begin, u8 needle) -> Option<usize> {
    for (auto index = begin; index < value.len(); ++index) {
        if (value[index] == needle) return Some(index);
    }
    return None();
}
...
auto inline_markdown(ref<str> value) -> String {
    ...
    for (auto index = usize {}; index < value.len();) {
        ...
        if (marker == u8('`') || marker == u8('*')) {
            ...
            if (close.is_none()) {
                result.push_str(escape_html(value.get(index, index + usize(1)).unwrap()).as_str());
                begin = index + usize(1);
                index = begin;
                continue;
            }
            result.push_str(marker == u8('`') ? "<code>"_str : "<em>"_str);
            result.push_str(escape_html(value.get(index + usize(1), *close).unwrap()).as_str());
            result.push_str(marker == u8('`') ? "</code>"_str : "</em>"_str);
            begin = *close + usize(1);
            index = begin;
            continue;
...
auto render_markdown(ref<str> value) -> String {
    ...
    while (begin <= bytes.len()) {
        ...
        if (line.starts_with("```"_str)) {
            ...
            output.push_ascii('\n');
        } else if (trimmed.starts_with("@brief "_str) || trimmed.starts_with("\\brief "_str)) {
            if (list) {
                output.push_str("</ul>"_str);
                list = false;
            }
            ...
            output.push_str(
                inline_markdown(trimmed.get(usize(7), trimmed.len()).unwrap()).as_str());
            output.push_str("</p>"_str);
        } else if (trimmed.starts_with("@return "_str) || trimmed.starts_with("@returns "_str) ||
                   trimmed.starts_with("\\return "_str) || trimmed.starts_with("\\returns "_str)) {
            if (list) {
                output.push_str("</ul>"_str);
                list = false;
            }
            auto offset =
                trimmed.starts_with("@returns "_str) || trimmed.starts_with("\\returns "_str)
                    ? usize(9)
                    : usize(8);
            output.push_str("<h3>Returns</h3><p>"_str);
            output.push_str(inline_markdown(trimmed.get(offset, trimmed.len()).unwrap()).as_str());
            output.push_str("</p>"_str);
        } else if (trimmed.starts_with("@param "_str) || trimmed.starts_with("@tparam "_str) ||
                   trimmed.starts_with("\\param "_str) || trimmed.starts_with("\\tparam "_str)) {
            if (list) {
                output.push_str("</ul>"_str);
                list = false;
            }
            auto offset = trimmed.starts_with("@param "_str) || trimmed.starts_with("\\param "_str)
                              ? usize(7)
                              : usize(8);
            ...
            auto text =
                split.is_some() ? body.get(*split + usize(1), body.len()).unwrap() : ref<str> {};
            output.push_str(template_parameter ? "<p><strong>Template parameter <code>"_str
            ...
            output.push_str(inline_markdown(line.get(usize(2), line.len()).unwrap()).as_str());
            output.push_str("</h2>"_str);
        } else if (line.starts_with("- "_str)) {
            if (! list) {
                output.push_str("<ul>"_str);
    ...
    if (list) output.push_str("</ul>"_str);
    if (code) output.push_str("</code></pre>"_str);
    return output;
}
